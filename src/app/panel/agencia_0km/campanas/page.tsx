...
export default async function Campanas0kmPage() {
  ...
  const [{ data: campaigns }, { data: channels }] = await Promise.all([
    ...
    supabase.from("channels").select("id, label").eq("area", AREA).eq("type", "whatsapp"),
  ]);
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Inicio de conversaciones con bases cargadas — Agencia 0KM.
          </p>
        </div>
        <NewCampaignForm channels={channels ?? []} redirectBase="/panel/agencia_0km/campanas" />
      </div>
      ...
      <div className="space-y-3">
        {(campaigns ?? []).map((c: any) => {
          ...
          return (
            ...
            >
              <Link href={`/panel/agencia_0km/campanas/${c.id}`} className="block min-w-0 flex-1">
        ...
        {(campaigns ?? []).length === 0 && (
          <p className="rounded-xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-400">
            Todavía no creaste ninguna campaña de Agencia 0KM.
          </p>
        )}
      </div>
    </div>
