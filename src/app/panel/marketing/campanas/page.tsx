...
export default async function CampanasMarketingPage() {
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Inicio de conversaciones con bases cargadas — Marketing.
          </p>
        </div>
        <NewCampaignForm channels={channels ?? []} redirectBase="/panel/marketing/campanas" />
      </div>
      ...
      <div className="space-y-3">
        {(campaigns ?? []).map((c: any) => {
          ...
          return (
            ...
            >
              <Link href={`/panel/marketing/campanas/${c.id}`} className="block min-w-0 flex-1">
        ...
        {(campaigns ?? []).length === 0 && (
          <p className="rounded-xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-400">
            Todavía no creaste ninguna campaña de Marketing.
          </p>
        )}
      </div>
    </div>
